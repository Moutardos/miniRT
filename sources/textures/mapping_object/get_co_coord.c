...
/*   Updated: 2024/04/22 01:52:40 by lcozdenm         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_texture_coordinates	get_co_coord(t_cone *cone, t_point_info *point_info)
{
	...
	if(are_doubles_equals(ft_dabs(perform_dot_product(point_info->normal, cone->vector)), 1))
		return (get_pl_coord(&cone->utils.induced_plane2, point_info));
	projected_position = rotate_vector_upside(cone->vector, point_info->point, cone->center);
